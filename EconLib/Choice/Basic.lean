...
theorem isFeasibleEquilibrium_profileOf_of_isOptimal
    (x : C.Choice)
    (h : C.IsOptimal x) :
    (C.toUtilityModel).IsFeasibleEquilibrium
      C.profileFeasible
      C.inducedFeasible
      (C.profileOf x) := by
  exact (C.isOptimal_iff_isFeasibleEquilibrium_profileOf x).mp h

theorem isOptimal_of_isFeasibleEquilibrium_profileOf
    (x : C.Choice)
    (h : (C.toUtilityModel).IsFeasibleEquilibrium
      C.profileFeasible
      C.inducedFeasible
      (C.profileOf x)) :
    C.IsOptimal x := by
  exact (C.isOptimal_iff_isFeasibleEquilibrium_profileOf x).mpr h
